...
#
# luci-app-footstrap-cmd/Makefile stays and is not a second source of truth: the lifecycle scripts
# are EXTRACTED from it below, so the two paths cannot disagree about what happens at install time.
set -eu

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SRC="$ROOT/luci-app-footstrap-cmd"
DIST="$ROOT/dist"
STAGE="$DIST/root"
...
extract() {			# <define-suffix> <outfile>
	awk -v want="define Package/luci-app-footstrap-cmd/$1" '
		$0 == want { in_block = 1; next }
		in_block && $0 == "endef" { exit }
		in_block { print }
	' "$SRC/Makefile" | sed 's/\$\$/$/g' > "$2"
	[ -s "$2" ] || {
		echo "stage: no Package/luci-app-footstrap-cmd/$1 block in the Makefile — refusing to" >&2
...
for d in "$SRC"/po/*/; do
	...
	cp "$d/footstrap-cmd.po" "$DIST/po-$lang/$lang/"
	printf "uci set luci.languages.%s='%s'; uci commit luci\n" \
		"$(echo "$lang" | tr - _)" "$label" \
		> "$DIST/i18n-$lang/etc/uci-defaults/luci-i18n-footstrap-cmd-$lang"
	chmod +x "$DIST/i18n-$lang/etc/uci-defaults/luci-i18n-footstrap-cmd-$lang"
done
